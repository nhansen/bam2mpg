# $Id: FASTA.pm 6773 2014-04-11 00:16:45Z deindorfersg $
package GTB::FASTA;
...
sub _read_seq {
    ...
    if ($self->{_use_samtools}) {
        ...
        my $line     = int($start / $rh->{bases_line});
        my $end_line = int($end / $rh->{bases_line});
        seek $fh, $line * $rh->{chars_line} + $rh->{offset}, 0;
        my $s = <$fh>;
        $s =~ s/\s+//g;
        my $pos = $start - $line * $rh->{bases_line};
        if ($end_line > $line) {
            $seq = substr $s, $pos;
            for (++$line;$line < $end_line;++$line) {
                $s = <$fh>;
                $s =~ s/\s+//g;
                $seq .= $s;
            }
